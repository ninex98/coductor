...
- 已完成：artifact lineage、resume stale 检测、codex exec fallback、动态 pipeline、contract stale 检测、安全 parallel 预检、parallel 审批恢复、worktree 并发执行、CLI 控制面真实 verify/review、运行 duration/token 指标、LangGraph checkpoint 生命周期清理、evidence hardening、demo E2E。
- 后续：Web 控制台、通知审批、PR 创建、更多 Backend、更细粒度的节点级幂等恢复。
