...
CLI 可观测性分三层：`coductor status [RUN_ID]` 查看运行总览；`coductor explain RUN_ID` 读取 SQLite checkpoint 并显示 `current_stage`、`completed_task_ids`、`last_error` 和 `stale_artifacts`；`coductor artifacts RUN_ID` 在列出 YAML Artifact 前也会显示同一份 checkpoint 摘要，方便把固定文件产物和当前运行状态对齐。
